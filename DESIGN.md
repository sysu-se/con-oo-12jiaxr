...
- 没有适配层，导致响应式更新困难。
- 领域对象代码放在 `src/node_modules/@sudoku/domain` 下，不符合常规项目结构。

#### HW1.1 的改进

1. **领域对象移至标准位置**：将 `domain` 文件夹从 `src/node_modules/@sudoku/domain` 移动到 `src/domain`，所有导入路径相应调整。
2. **引入 Store Adapter**（`gameInstance.js`）：
   - 封装 `Game` 实例的创建和访问。
...
8. **增加历史长度限制**（`MAX_HISTORY = 100`），防止内存无限增长。
...
- **合理组织代码**：领域对象移至 `src/domain`，符合常规项目结构。
